...
const server = http.createServer((req, res) => {
  ...
  } else if (req.method === 'GET' && parsed.pathname && parsed.pathname.startsWith('/attestations/')) {
    const rest = parsed.pathname.replace('/attestations/', '');
    const file = path.join(__dirname, '../fixtures', rest.replace(/\\/g, '-') + '.json');
    const fallback = path.join(__dirname, '../fixtures', rest.split('/').join('-') + '.json');
    const target = fs.existsSync(file) ? file : fallback;
    if (!fs.existsSync(target)) {
      res.writeHead(404, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ error: 'no attestation', package: rest }));
      return;
    }
    res.writeHead(200, { 'Content-Type': 'application/vnd.in-toto+json' });
...
server.listen(PORT, '127.0.0.1', () => {
  ...
  console.log('  GET    /attestations/widget-lib/1.0.0');
