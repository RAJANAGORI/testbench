...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'POST' && parsed.pathname === '/collect') {
  ...
  } else if (req.method === 'GET' && parsed.pathname === '/catalog') {
    const catalog = JSON.parse(fs.readFileSync(path.join(__dirname, 'catalog-fixture.json'), 'utf8'));
    ...
    const exists = Array.isArray(catalog.packages) && catalog.packages.includes(String(name));
    res.writeHead(exists ? 200 : 404, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ name, exists, check: exists ? 'in-catalog' : '404-not-in-catalog' }));
    } else {
    res.writeHead(404);
    res.end('Not Found');
  }
