...
#
# Front-door installer for SCAS:
#   - interactive menu: labs only, workshop stack, or Docker labs
#   - prerequisites (Node, npm, Python, Git, Docker, curl)
#   - TESTBENCH_MODE via scripts/setup/setup.sh, then npm workspaces
#   - workshop: Elasticsearch + Kibana + Floci + lookalike lab secrets
#   - always writes .scas.env
#
# Usage:
#   ./install.sh                 # interactive menu: labs / workshop / docker
#   ./install.sh -y              # non-interactive workshop stack (npm + ES + Floci)
#   ./install.sh -y --core-only  # labs only: setup.sh + npm (no Docker services)
...
#   source .scas.env
#   First lab: cd scenarios/01-typosquatting && ./setup.sh
...
FLOCI_IMAGE=1
START_UI=0
INSTALL_KIND="" # labs | workshop | docker
...
if [ "$CORE_ONLY" = "1" ]; then
  ...
  SKIP_FLOCI=1
  INSTALL_KIND=labs
elif [ "$YES" = "1" ] || [ "$SKIP_ES" = "1" ] || [ "$SKIP_FLOCI" = "1" ] || [ "$NO_START" = "1" ] || [ "$FLOCI_IMAGE" = "0" ] || [ "$START_UI" = "1" ]; then
  INSTALL_KIND=workshop
fi
...
write_scas_env() {
  local SCAS_ENV_FILE="${REPO_ROOT}/.scas.env"
  {
    cat << 'EOF'
...
pick_install_kind() {
  if [ -n "$INSTALL_KIND" ]; then
    if [ "$YES" != "1" ]; then
      if ! ask_yn "Continue with SCAS install?"; then
  ...
  echo ""
  echo "  1) Labs only      Node/Python. No Docker. Fast path."
  echo "  2) Workshop stack npm workspaces, Elasticsearch/Kibana, Floci."
  echo "  3) Docker labs    exec ./docker/install.sh"
  echo ""
  local choice
...
MISSING=0
NEED_DOCKER=0
if [ "$INSTALL_KIND" = "docker" ]; then
  NEED_DOCKER=1
elif [ "$SKIP_ES" != "1" ] || [ "$SKIP_FLOCI" != "1" ]; then
  NEED_DOCKER=1
fi
...
if [ "$INSTALL_KIND" = "docker" ]; then
  log "Docker labs: writing .scas.env, then exec ./docker/install.sh"
...
log "Calling scripts/setup/setup.sh -y (TESTBENCH_MODE, dirs, minimal .scas.env)…"
...
# ── 8. Optional UI ───────────────────────────────────────────
if [ "$START_UI" != "1" ] && [ "$YES" != "1" ] && [ "$NO_START" != "1" ]; then
  if ask_yn "Start the Control Center now?"; then
    START_UI=1
  fi
fi
...
echo "  2. First lab:  cd scenarios/01-typosquatting && ./setup.sh"
echo "     Walkthrough: documentation/scenario-guides/zero-to-hero/ZERO_TO_HERO_SCENARIO_01.md"
echo "     Short guide: documentation/getting-started/ZERO_TO_HERO.md"
echo "  3. Optional UI: ./scripts/ui/start-dashboard.sh"
echo "     Landing     http://localhost:5173"
echo "     Dashboard   http://localhost:3100"
echo ""
if [ "$SKIP_FLOCI" != "1" ] || [ "$SKIP_ES" != "1" ]; then
  echo "Shutdown (if you started the workshop stack):"
  [ "$SKIP_FLOCI" != "1" ] && echo "  ./scripts/floci/floci-down.sh"
  [ "$SKIP_ES" != "1" ] && echo "  ./scripts/observability/elasticsearch-down.sh"
