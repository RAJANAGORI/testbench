...
#   ./install.sh -y --with-ui    # exec ./scripts/ui/start-dashboard.sh at the end
#
...
# Generated by install.sh / scripts/setup/setup.sh
export TESTBENCH_MODE=enabled
EOF
fi
...
if [ "$SKIP_FLOCI" = "1" ]; then
  ...
  if [ "$FLOCI_IMAGE" = "1" ]; then
    ...
    ./scripts/floci/floci-setup.sh --auto
  else
  ...
  if [ "$NO_START" != "1" ]; then
    ...
    if ./scripts/floci/floci-up.sh; then
      FLOCI_UP_OK=1
    else
    ...
    if [ "$FLOCI_UP_OK" = "1" ]; then
      ...
      echo "           ./scripts/floci/floci-up.sh"
      echo "   Or skip: ./install.sh -y --skip-floci"
      # Keep going — do not abort ES/npm/lookalike setup
      SKIP_FLOCI_SEED=1
    fi
  else
    warn "Floci installed but not started (--no-start). Later: ./scripts/floci/floci-up.sh"
  fi
fi
...
if [ "$SKIP_FLOCI" != "1" ] && [ "$NO_START" != "1" ] && [ "${SKIP_FLOCI_SEED:-0}" != "1" ]; then
  ...
  if scas_floci_require 2>/dev/null; then
    ...
    warn "Floci not healthy — SM/SSM lookalikes not seeded. Later: ./scripts/floci/floci-up.sh then re-run seed"
  fi
elif [ "$SKIP_FLOCI" != "1" ] && { [ "$NO_START" = "1" ] || [ "${SKIP_FLOCI_SEED:-0}" = "1" ]; }; then
  warn "Floci SM/SSM lookalikes deferred. After Floci is up: ./scripts/floci/floci-up.sh && scenario infrastructure/floci/seed.sh"
fi
...
if [ "$START_UI" = "1" ]; then
  if [ "$NO_START" = "1" ]; then
    ...
    log "Starting Control Center UI (./scripts/ui/start-dashboard.sh)…"
    echo "   (run in a separate terminal if you prefer — this script will exec the UI starter)"
    exec ./scripts/ui/start-dashboard.sh
  fi
fi
...
echo "  2. Optional UI:     ./scripts/ui/start-dashboard.sh"
...
echo ""
echo "Shutdown:"
echo "  ./scripts/floci/floci-down.sh"
echo "  ./scripts/observability/elasticsearch-down.sh"
