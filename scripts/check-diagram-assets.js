...
/**
 * Harness: enforce hand-authored diagram drawing contract.
 *
 * Specs: scripts/lib/diagram-specs.js
 * Assets: documentation/assets/diagrams/<basename>.{excalidraw,svg}
 *
 * Usage (repo root):
...
function assertPair(spec) {
  const ex = `${DIAGRAMS_DIR}/${spec.basename}.excalidraw`;
  const svg = `${DIAGRAMS_DIR}/${spec.basename}.svg`;
...
function assertMarkdownContract(spec) {
  ...
  for (const doc of docs) {
    ...
    const text = read(doc);
    const embed = `assets/diagrams/${spec.basename}.svg`;
    if (!text.includes(embed)) {
      fail(`${doc}: must embed ${embed}`);
    } else {
      embedHits += 1;
    }
    ...
    const match = text.match(
      new RegExp(
        `!\\[[^\\]]*\\]\\((\\.\\./(?:\\.\\./)?assets/diagrams/${spec.basename}\\.svg)\\)`
      )
    );
    if (!match) {
      fail(`${doc}: image embed path for ${spec.basename}.svg not found or wrong relative form`);
    } else {
...
function main() {
  ...
  if (failures.length) {
    ...
    console.error('Then re-run: node scripts/check-diagram-assets.js');
    process.exit(1);
  }
